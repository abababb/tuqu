...
func fetchPost(post Post) []Reply {
	...
	reAuthorname := regexp.MustCompile(`№(?P<reply_no>[[:digit:]]+)☆☆☆(?P<author_name>.*?)\|?(?P<author_code>[[:alnum:]]{8})?于(?P<reply_time>[[:digit:]]{4}-[[:digit:]]{2}-[[:digit:]]{2} [[:digit:]]{2}:[[:digit:]]{2}:[[:digit:]]{2})留言☆☆☆`)
